...
void LoadingScreen::SerialiseComponent(Serialiser& document)
{
	...
	if (document.HasMember("L.CameraLinkId") && document["L.CameraLinkId"].IsInt())
		_panningCameraLinkId = document["L.CameraLinkId"].GetInt();
...
{
	GetSibilingComponentObject(Graphic)->SetFileName(_FileName);
	...
	if (_loadingCompleted)
	{
		...
		{
			_camera->SetMainCamera(true);
			_cameraTransform->SetPos(GetSibilingComponentObject(Transform)->GetPositionA());
			_camera->_cameraZoom = 1.f;
...
void LoadingScreen::Update(double dt)
{
	if (_panning && !_panningComleted)
	{
		...
		_camera->_cameraZoom += _zoomSpeed * MyFrameRateController.Getdt() * _panningSpeed;
		...
		if (_camera->_cameraZoom < _endZoom)
		{
			...
			if(_secondImage)
				GetSibilingComponentObject(Graphic)->SetFileName(_FileName2);
