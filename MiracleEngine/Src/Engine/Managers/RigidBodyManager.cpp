...
void RigidbodyManager::ApplyVelocityToObject(double dt)
{
	for (auto it : _rigidBody2dList)
	{
		...
		_GlobalContainer._transforManager->GetTransform(it.first)->GetPos() += it.second->_velocity * (float)dt;
	}
}
...
{
	RigidBody2D* object = _GlobalContainer._rigidbodyManager->_rigidBody2dList[uId];
...
{
	RigidBody2D* object = _GlobalContainer._rigidbodyManager->_rigidBody2dList[uId];
	TransformComponent* transform = _GlobalContainer._transforManager->GetTransform(uId);
