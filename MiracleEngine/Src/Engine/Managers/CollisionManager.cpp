...
void CollisionManager::UpdateStaticCollision(double dt)
{
		...
		tempList.erase(it))
	{
		...
		TransformComponent* transform = _GlobalContainer._transforManager->GetTransform(it->first);
...
{
	TransformComponent* transform = _GlobalContainer._transforManager->GetTransform(collider1->GetParentId());
	TransformComponent* transform2 = _GlobalContainer._transforManager->GetTransform(collider2->GetParentId());

	RigidBody2D* rigidbody = nullptr;
	RigidBody2D* rigidbody2 = nullptr;

	if (collider1->_attachedRigidboy)
		rigidbody = _GlobalContainer._rigidbodyManager->_rigidBody2dList[collider1->GetParentId()];

	if (collider2->_attachedRigidboy)
		rigidbody2 = _GlobalContainer._rigidbodyManager->_rigidBody2dList[collider2->GetParentId()];
...
void CollisionManager::UpdateColliderData(Collider2D* collider)
{
	...
	case (unsigned)ColliderType::EDGE_COLLIDER:
	{
		...
		TransformComponent* transform = _GlobalContainer._transforManager->GetTransform(collider->GetParentId());
