...
bool ImGuiFunctions::ComboFilter__DrawPopup(std::string comboName, ComboFilterState& state, int START, std::vector<const char*> ENTRIES, int ENTRY_COUNT, std::string& _fileName, bool* open, int i)
{
	...
	std::string beginName = "##" + comboName + std::to_string(i);
...
bool ImGuiFunctions::ComboFilter(const char* id, char* buffer, int bufferlen, std::vector<const char*> hints, int num_hints, ComboFilterState& s, std::string& _filename, bool* open, int i)
{
	struct fuzzy {
	...
	if (hot) {
		...
		s.activeIdx = idx;
		bool hello = true;
		if (*open || done || (hello = ComboFilter__DrawPopup(id, s, idx, hints, num_hints, _filename, open, i))) {
			//*open = false;
			int i = s.activeIdx;
			if (i >= 0) {
