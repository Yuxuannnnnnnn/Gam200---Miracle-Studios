...
void AISystem::CreateNodeMap()
{
	...
	_GlobalContainer._collisionManager->_collisionMap.AddNewMap(
	...
	for (int y = 0; y < (int)_mapHeight; ++y)
	{
		for (int x = 0; x < (int)_mapWidth; ++x)
		{
			...
			{	// create wall only if solid
				tempGo = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["Wall"]);
				//tempGo->Set_typeId(TypeIdGO::WALL);
				((TransformComponent*)tempGo->GetComponent(ComponentId::TRANSFORM_COMPONENT))->SetPos(tempVec);
				
				_GlobalContainer._collisionManager->_collisionMap.AddNewTile(y, x, MapTile{ TileType::HARD_WALL, tempGo->Get_uID() });
				//std::cout << id;
			}
			else if (spawner)
			{	// create spawner if tileMapInput[][] == 2
				GameObject* spawner = nullptr;
				spawner = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["Spawner"]);
			...
			{	// create spawner if tileMapInput[][] == 3
				GameObject* spawner = nullptr;
				spawner = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["SpawnerTwo"]);
				((TransformComponent*)spawner->GetComponent(ComponentId::TRANSFORM_COMPONENT))->SetPos(tempVec);
			...
			{	// create spawner if tileMapInput[][] == 4
				GameObject* spawner = nullptr;
				spawner = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["SpawnerThree"]);
				((TransformComponent*)spawner->GetComponent(ComponentId::TRANSFORM_COMPONENT))->SetPos(tempVec);
			...
			{	// create wall only if solid
				tempGo = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["Floor"]);
	...
	size_t tilesize = _GlobalContainer._aiSystem->GetMapTileSize();
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ 0.0f * tilesize, 0.0f * tilesize,1 }, { 900,900,1 }, 4.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ -7.0f * tilesize, 5.0f * tilesize,1 }, { 500,500,1 }, 2.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ -10.0f * tilesize, -8.0f * tilesize,1 }, { 1200,900,1 }, 3.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ 10.0f * tilesize, 7.0f * tilesize,1 }, { 1200,900,1 }, 1.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ 12.0 * tilesize, -7.0f * tilesize,1 }, { 500,600,1 }, 5.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ -12.0 * tilesize, 7.0f * tilesize,1 }, { 700,700,1 }, 6.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ -13.0 * tilesize, 0.0f * tilesize,1 }, { 1000,400,1 }, 7.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ 3.0 * tilesize, -10.0f * tilesize,1 }, { 800,1000,1 }, 9.0f);
	//_GlobalContainer._graphicsSystem->DrawRockyTile({ 1.0 * tilesize, 10.0f * tilesize,1 }, { 600,1000,1 }, 11.0f);

	//_GlobalContainer._graphicsSystem->DrawCircularBatteryPlatform({ -5.3f * tilesize,-3.5f * tilesize,1 }, { 900,900,1 },4.0f);
	//_GlobalContainer._graphicsSystem->DrawBuilding2({ 0.0f * tilesize, 3.0f * tilesize,1 }, { 550, 950,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding4({ 4.0f * tilesize, 8.5f * tilesize,1 }, { 280, 330,1 }, PI/2);
	//_GlobalContainer._graphicsSystem->DrawCircularBatteryPlatform({ 5.5f * tilesize, 11.5f * tilesize,1 }, { 400, 400,1 },1.0f);
	//_GlobalContainer._graphicsSystem->DrawBuilding2({ 10.0f * tilesize, 9.9f * tilesize,1 }, { -450, -580,1 });
	//_GlobalContainer._graphicsSystem->DrawCircularBatteryPlatform({ 11.9f * tilesize, 0.9f * tilesize,1 }, { 1080, 1080,1 },3.0f);
	//_GlobalContainer._graphicsSystem->DrawBuilding1({ -6.0f * tilesize, 9.8f * tilesize,1 }, { 1150, 900,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding2({ -15.5f * tilesize, 10.9f * tilesize,1 }, { -650, -580,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding2({ -15.5f * tilesize, 5.1f * tilesize,1 }, { 650, 580,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding4({ -12.9f * tilesize, -11.0f * tilesize,1 }, {430,780,1 }, PI/2);
	//_GlobalContainer._graphicsSystem->DrawBuilding3({ 11.5f * tilesize, -9.5f * tilesize,1 }, { 1000, 1000,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding4({ 7.5f * tilesize, -9.5f * tilesize,1 }, { 500, 680,1 });
	//_GlobalContainer._graphicsSystem->DrawBuilding4({ 15.5f * tilesize, -9.5f * tilesize,1 }, { 500, 680,1 });



	GameObject * obj = _GlobalContainer._gameObjectFactory->
		CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["MapEdge"]);
	...
	com->SetRotate(((const float)PI));
	 ...
	 obj = _GlobalContainer._gameObjectFactory->
		CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["MapEdge"]);
...
void AISystem::CreateNodeMapFromTileComp()
{
	...
	if ((_GlobalContainer._gameObjectFactory->getTileMapComponents()).begin() == _GlobalContainer._gameObjectFactory->getTileMapComponents().end())
		return;
	for (auto it : _GlobalContainer._gameObjectFactory->getTileMapComponents())
		if (it.first >= 1000)
		{
	...
	for (int y = 0; y < _mapHeight; ++y)
	{
		...
		//	solid = _tilemapInput[y][x] == 1 ? true : false; // need 'palette' to determine solidty
			...
			GameObject* tempGO = _GlobalContainer._gameObjectFactory->CloneGameObject(_GlobalContainer._prefabFactory->GetPrototypeList()["Floor"]);
			((TransformComponent*)tempGO->GetComponent(ComponentId::TRANSFORM_COMPONENT))->SetPos(tempVec);
