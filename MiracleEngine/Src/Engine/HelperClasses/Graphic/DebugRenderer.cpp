...
DebugRenderer::DebugRenderer()
{
	...
	_proj = glm::ortho(-(float)_GlobalContainer._windowSystem->getWindow().GetWindowWidth() / 2, (float)_GlobalContainer._windowSystem->getWindow().GetWindowWidth() / 2,
		-(float)_GlobalContainer._windowSystem->getWindow().GetWindowHeight() / 2, (float)_GlobalContainer._windowSystem->getWindow().GetWindowHeight() / 2, -15.0f, 15.0f);
...
void DebugRenderer::DrawCircle(float x, float y, float radiusin)
{
	...
	////glm::mat4 mvp = _proj * _GlobalContainer._graphicsSystem->GetCamera().GetCamMatrix() * model;
...
void DebugRenderer::SubmitDebugLine(float x1, float y1, float x2, float y2)
{
	...
	//angle += 0.01f;
	////glm::mat4 mvp = _proj * _GlobalContainer._graphicsSystem->GetCamera().GetCamMatrix();
...
void DebugRenderer::BatchDrawDebugLine()
{
	...
	//glm::mat4 view = _GlobalContainer._graphicsSystem->GetCamera().GetCamMatrix();
	//glm::mat4 mvp = _proj * _GlobalContainer._graphicsSystem->GetCamera().GetCamMatrix();
	//
...
void DebugRenderer::DrawWireFrameQuad(int xpos, int ypos, int xsize, int ysize)
{
	...
	//glm::mat4 model = translate * _GlobalContainer._graphicsSystem->GetCamera().GetCamMatrix() * glm::scale(glm::mat4(1.0f), glm::vec3(xsize, ysize, 1.0f));
