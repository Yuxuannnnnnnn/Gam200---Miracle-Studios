...
{
	return Vector3{ (float)(_p.x - (EngineSystems::GetInstance()._windowSystem->getWindow().GetWindowWidth()) / 2),
	  (float)(EngineSystems::GetInstance()._windowSystem->getWindow().GetWindowHeight() / 2) - _p.y, 1 };
}
