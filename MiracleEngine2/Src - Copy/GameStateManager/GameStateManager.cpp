...
void GameStateManager::SetNextGameState(GameStateId state)
{
	_nextState = state;
}

bool GameStateManager::CheckIfCurrStateQuit() const
{
	return _currState != GameStateId::GS_QUIT;
}

bool GameStateManager::CheckIfCurrNextStateSame() const
{
	return _currState == _nextState;
}
